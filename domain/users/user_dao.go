...
import (
	"fmt"
	"github.com/harshasavanth/bookstore_users-api/datasources/mysql/users_db"
	"github.com/harshasavanth/bookstore_users-api/logger"
	"github.com/harshasavanth/bookstore_users-api/utils/errors"
	"github.com/harshasavanth/bookstore_users-api/utils/mysql_utils"
	"strings"
)
...
func (user *User) Save() *errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to get ast inserted ID ", err)
		return errors.NewInternalServerError("database error")
	}
	user.Id = userId
	return nil
...
func (user *User) FindByStatus(status string) ([]User, *errors.RestErr) {
	...
	if len(results) == 0 {
		return nil, errors.NewNotFoundError(fmt.Sprintf("no user matching criteria: %s", status))
	}
	return results, nil
}
